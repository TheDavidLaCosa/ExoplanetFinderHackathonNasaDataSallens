...
def health():
    ...
    return jsonify({"status": "ok", "message": "Backend Features API is running"})

@app.route('/api/upload', methods=['POST'])
def upload_file():
    """Upload and clean file using backend_features logic"""
...
def analyze_data():
    ...
    try:
        ...
        try:
            data_processed, data_pac, model = process_data(list(df_filtered.columns), level=20)
            ...
            return jsonify({
                ...
                "ml_processed": True,
                "plots_saved": [
                    "./img/Correlation_numeric_values.jpg",
                    "./img/80_per_cent_PCA.jpg",
                    "./img/Correlation_numeric_values_pca.jpg"
                ]
            })
...
def get_columns():
    """Get available columns for a specific upload"""
    upload_id = request.args.get('upload_id')
    if not upload_id or upload_id not in CLEANED_CSV_PATHS:
...
if __name__ == '__main__':
    _ensure_upload_dir()
    print("\n" + "="*60)
    print("🚀 NASA DataPilot Backend Features API")
    print("="*60)
    ...
    print(f"📋 Columns endpoint: http://localhost:4000/api/columns")
    print("="*60 + "\n")
